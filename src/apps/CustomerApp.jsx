...
export default function CustomerApp({ pricing, openDriver, openAdmin, toast }) {
  ...
  const historySeed = [
    { route: "Перник → София", service: "RIVO Premium · BMW 530d Touring", price: 34, date: "Днес, 09:40" },
    { route: "Перник · Център", service: "RIVO City · Audi A1", price: 8, date: "Вчера, 18:20" }
  ];
  ...
  if (screen === "confirm") return (
    <>
      ...
      <main className="content">
        ...
        <RouteSummary destination={destination} timing={timing} passengers={passengers} luggage={luggage} payment={payment}/>
        <Card className="confirmVehicle"><div><span className="eyebrow">ТВОЯТ АВТОМОБИЛ</span><h2>{selected.label}</h2><small>{selected.model} · до 4 места</small></div><VehicleImage src={selected.image} alt={selected.model}/></Card>
  ...
  if (screen === "trip") return (
    <>
      ...
      <main className="content">
        <div className="statusHeader"><div><span className="eyebrow">RIVO PREMIUM · В ПЪТ</span><h1>{Math.max(1, 16 - Math.floor(tripProgress / 7))} мин до {destination.split(",")[0]}</h1><p className="muted">Пристигаме спокойно и навреме.</p></div><span className="liveDot"/></div>
